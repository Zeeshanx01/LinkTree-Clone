...
const Navbar = () => {
  ...
  // const showNavbar = ['/', '/generate', `/generate?handle=${handle}`].includes(normalizedPathName)

  // const showNavbar = ['/', 'generate', `/generate?handle=${handle}`].includes(pathName)


  return (
    showNavbar && <nav className='w-[80vw] h-[8vh] z-10 flex justify-between items-center bg-white rounded-full mx-auto p-3 fixed top-10 right-[10vw] hover:w[90vw] hover:right[5vw] transition-all duration-300 drop-shadow-2xl'>
      ...
      <div className="buttons space-x-4">
        ...
        <button className='bg-gray-900 hover:bg-gray-800 text-white text-lg font-bold px-6 py-4 rounded-full'>Sign up free</button>
      </div>
